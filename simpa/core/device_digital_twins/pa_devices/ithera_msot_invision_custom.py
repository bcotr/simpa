...
class InVision256TF_custom(PhotoacousticDevice):
    """
    ...
    def __init__(self, device_position_mm: np.ndarray = None,
                 field_of_view_extent_mm: np.ndarray = np.asarray([-20, 20, 0, 0, -20, 20]), number_detectors=256):
        """
        ...
        """
        super(InVision256TF_custom, self).__init__(device_position_mm=device_position_mm,
                                            field_of_view_extent_mm=field_of_view_extent_mm)
        pitch_mm = 0.735 * (256/number_detectors)        

        self.number_detectors = number_detectors
        detection_geometry = CurvedArrayDetectionGeometry(pitch_mm=pitch_mm,
                                                          radius_mm=40,
    ...
    def serialize(self) -> dict:
        ...
        device_dict = {"InVision256TF_custom": serialized_device}
        return device_dict

    @staticmethod
    def deserialize(dictionary_to_deserialize):
        deserialized_device = InVision256TF_custom()
